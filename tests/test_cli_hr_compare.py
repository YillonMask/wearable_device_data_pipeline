...
def _seed_with_strap(conn):
    create_hr_session(
        conn, session_id="S2", label="bike",
        started_at="2026-07-21T18:30:05+00:00",
        devices=["strap", "whoop", "google_health"],
    )
    rows = []
    for sec in range(10):
        ts = f"2026-07-21T18:30:{5 + sec:02d}+00:00"
        rows.append(("S2", "strap", ts, sec * 1000, 120 + sec))
        rows.append(("S2", "whoop", ts, sec * 1000, 121 + sec))
    for sec in range(0, 10, 3):
        ts = f"2026-07-21T18:30:{5 + sec:02d}+00:00"
        rows.append(("S2", "google_health", ts, sec * 1000, 117 + sec))
    insert_hr_samples(conn, rows)
...
def test_hr_compare_uses_strap_as_baseline(monkeypatch, tmp_path):
    monkeypatch.setenv("DATABASE_PATH", str(tmp_path / "t.db"))
    from wearable_pipeline import config
    monkeypatch.setattr(cli, "load_settings", config.load_settings)
    ...
    conn = db.connect(tmp_path / "t.db")
    db.migrate(conn)
    _seed_with_strap(conn)
    conn.close()

    out = tmp_path / "chart2.png"
    result = runner.invoke(cli.app, ["hr-compare", "S2", "--out", str(out)])
    assert result.exit_code == 0, result.output
    assert "baseline: strap" in result.output
    assert "whoop vs strap" in result.output
    assert "google_health vs strap" in result.output
    assert out.exists() and out.stat().st_size > 0
